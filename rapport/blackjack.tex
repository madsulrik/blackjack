...
\section{Problemformulering} \label{sec:problem}
  ...
  og beskrive hvordan vi har forstået den udleveret kravspecifikation.

  I dette projekt vil vi gerne udvikle et program der kan spilles af 1-5 spillere i kommandopromten,
  med mulighed for AI. Der er implementeret i F\# og følger reglerne for SimpleJack der er beskrevet i afsnittet Afsnit \ref{ssec:demands} - \nameref{ssec:demands}.
  ...
  \section{Programbeskrivelse} \label{sec:programDescription}
    ...
    De følgende funktioner kan findes i filen \path{game.fsx}:
    ...
    \paragraph*{printScoreboard)}

    \paragraph*{selectPlayer)}
    ...
    \subsection{Hjælpefunktioner} \label{ssec:helpers}
      De følgende hjælpefunktioner kan findes i filen \path{game.fsx}:
      \paragraph*{validate name)}
        Validere om længden af navnet(string) er større end nul,
        og mindre end 25.
      
      \paragraph*{validate yn)}
        Validere om inputtet er i mængden {y, n} af strings.
      
      De følgende hjælpefunktioner kan findes i filen \path{blackjack.fsx}:
